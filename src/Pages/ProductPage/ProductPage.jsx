  ...
  const fetchProducts = async () => {
    ...
    try {
      const res = await Api.get(
        `api/product/admin?page=${page}&search=${search}&filter=${filter}`,
        {
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="text-xl leading-6 font-medium">Product Management</h1>
          ...
          <div className="flex">
            <div className="relative  w-[584px] h-10 ">
              ...
              <input
                ...
                className="border border-[#D5D5D5] pl-10 p-[10px] placeholder:text-[#C1C1C1] focus:outline-none hover:border-black  font-normal text-sm leading-5 rounded-lg w-full h-10"
              />
            </div>
            <div className="relative  h-10 ml-4">
              ...
              <select
                ...
                className="appearance-none pl-9 text-[#2C2B2B] hover:border-black duration-300 font-normal  pr-3 border w-full border-[#D5D5D5] h-10 rounded-lg text-sm focus:outline-none"
              >
            ...
            <table className="min-w-full text-sm text-left">
              <thead className="border-b font-semibold border-[#D9D9D9] text-[#252525]">
                <tr>
                  ...
                  <th className="py-4 px-4">Product Detail</th>
              ...
              <tbody>
                {products.map((prod) => (
                  ...
                  >
                    <td className="py-4 px-4">MYO{prod.id}</td>
                    <td className="py-4 px-4">{prod.name}</td>
                    <td className="py-4 px-4">₹{prod.totalPrice}</td>
                    <td className="py-4 px-4">{prod.stockQuantity}</td>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40">
          ...
          <img src={deleteWarning} className="w-12 h-12" alt="" />
            <p className="mt-4 text-[#030300] text-sm leading-4 font-semibold">
            Confirm delete item?
            </p>
            <p className="mt-2 text-[#818180] text-sm font-normal leading-6"> Are you sure you want to delete the product ? <br />
            This action cannot be undone.</p>
            <div className="flex mt-4 gap-4">
            <button
                onClick={() => setShowDeleteModal(false)}
                className="border-[#D5D5D5] hover:border-[#050505] duration-300 border w-full text-base font-medium px-4 py-2 rounded-lg"
              >
                Cancel
              </button>
              <button
                onClick={handleDelete}
                className="bg-[#C30000] hover:bg-[#DE5555] duration-300 text-white w-full text-base font-medium px-4 py-2 rounded-lg"
              >
                Delete
              </button>
